...
const ChatWithUser = ({ route }) => {
  ...
  const { userId, chats, _id } = chat;

  useLayoutEffect(() => {
    navigation.setOptions({
      headerTitle: `${userId.firstName + " " + userId.lastName}`,
      headerStyle: {
  ...
  const [clientAttachment, setClientAtachment] = useState({
    uri: "",
    name: "",
    type: "",
  });

  const getMyChatwithUser = async () => {
    const headers = { headers: { Authorization: `Bearer ${clientToken}` } };

    const _id = chatRoomId;

    const response = await getMychatWithUserService({ _id, headers });
  ...
  const getMyChatofUseronLoad = async () => {
    setChatload(true);
    const headers = { headers: { Authorization: `Bearer ${clientToken}` } };
  ...
  const postAttachment = async () => {
    ...
    console.log("clientAttachment: ", clientAttachment);
    ...
    fd.append("clientAttachment", clientAttachment);
    ...
    const response = await postClientAttachment({ fd, headers });
